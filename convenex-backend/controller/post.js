...
exports.addCommunityPost = async (req, res) => {
    try {
        ...
        const post = await Post.create({ user: req.user._id, community: communityId, desc, imageLink });
        if (!post) {
            return res.status(400).json({ error: "Something went wrong" });
        }
        return res.status(200).json({
...
exports.getPostByPostId = async (req, res) => {
    try {
        ...
        let post = await Post.findById(postId).populate("user community", "-password");//createdAt is usually a timestamp field that stores when a document was created in MongoDB. In Mongoose, it commonly appears automatically when you enable timestamps:true. createdAt: -1 → newest posts first (descending order). createdAt: 1 → oldest posts first (ascending order)
        if (!post) {
            return res.status(400).json({ error: "Post not found." });
        }
        return res.status(200).json({
...
exports.getAllPostsOfCommunity = async (req, res) => {
    try {
        const { communityId } = req.params;
        let allPosts = await Post.find({ community: communityId }).sort({ createdAt: -1 }).populate("community user","-password");
        return res.status(200).json({
            message: "Fetched all posts of community.",
            posts: allPosts
        });
    } catch (err) {
        console.error(err);
