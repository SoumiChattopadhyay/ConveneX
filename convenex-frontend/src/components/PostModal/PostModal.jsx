...
const PostModal = (props) => {
    ...
    const handlePost = async (event) => {
        try {
            ...
            await axios.post("http://localhost:4000/api/post/addPost", { desc: desc, imageLink: imgUrl }, { withCredentials: true }).then(res => {
    ...
    return (
        <div>
            ...
            <div className='flex items-center gap-2 py-3 px-3'>
                <div>
                    <img className='w-11 h-11 rounded-4xl' src={props.personalData?.profilePic} alt="profile-icon" />
                </div>
                <div className='font-semibold text-md'>{props.personalData?.f_name}</div>
            </div>
            ...
            {
                imgUrl && <div className='p-3'>
                    <img className='w-10 h-10 rounded-lg' src={imgUrl} alt="post-img" />
                </div>
            }
